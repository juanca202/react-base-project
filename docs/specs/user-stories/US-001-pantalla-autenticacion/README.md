...
- **Diseño / prototipo:** [Pantallas taller SDD — nodo de referencia](https://www.figma.com/design/7pt2W7JSic4ZoAVcgvQ5qD/Pantallas-taller-SDD?node-id=1-3167&m=dev) (archivo `7pt2W7JSic4ZoAVcgvQ5qD`, nodo `1-3167` en modo Dev).
...
| **T** | Testeable     | Cumple    | El alcance de pruebas en **Observaciones** fija el foco obligatorio de verificación (rutas protegidas, login ya autenticado y cierre de sesión con destino único al login). Otros escenarios Gherkin pueden comprobarse según criterio del equipo. Amenazas de seguridad de producción quedan fuera del alcance. |
...
| Sin aclaraciones pendientes        | Cumple | Lo pendiente queda formalizado en **RN-04** (catálogo de rutas por definir), **RN-06** (mocks e integración futura) y **Observaciones**.                                                                                                                                                |

## Observaciones

- **Alcance de las pruebas:** el foco obligatorio de verificación, con mocks según **RN-06**, es: (1) **mecanismo de rutas protegidas** — con al menos una ruta de referencia, quien no tiene sesión válida **NO DEBE** acceder a su contenido y **DEBE** ser redirigido al login (**RN-04**, primer escenario Gherkin); (2) **login estando ya autenticado** — el sistema **DEBE** redirigir al resumen (**RN-03**, tercer escenario Gherkin); (3) **cierre de sesión en cualquier flujo** — el usuario **DEBE** quedar fuera de sesión y ser llevado **siempre** al login (**RN-05**, cuarto escenario Gherkin).
- Catálogo concreto de rutas protegidas: pendiente de definición por producto (**RN-04**).
- Integración con backend o proveedor de identidad real: fuera de alcance actual; ver **RN-06**.
