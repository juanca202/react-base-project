...
- _Entrada:_ «Implementa lo Ready de la US-042; estoy en el paquete `@acme/web-app`.»
- _Salida:_ Rama limpia y checkout a `feature/US-042-[nombre-corto]`; mensaje con TK Ready del paquete en cola y excluidas con `(Draft)` / `(Skipped)`; tras confirmación, implementa la primera Ready, ejecuta lint/build, actualiza `progress.md`, pregunta por la siguiente. Sin tests hasta el cierre.
  **Ejemplo 2 — TK indicada sin US**

- _Entrada:_ «Implementa TK-003.»
- _Comportamiento:_ El agente pregunta a qué `US-XXX` pertenece. El usuario responde «US-042». El agente valida que `TK-003-[nombre].md` existe dentro de `docs/specs/user-stories/US-042-[nombre-corto]/`. Si existe, continúa con el flujo normal. Si no existe o pertenece a otra US, para con el mensaje de error estructurado indicando el motivo.
  **Ejemplo 3 — TK en Draft**

- _Entrada:_ «Ejecuta TK-005 de la US-042» y TK-005 está en Draft.
- _Salida:_ Lista de excluidas: `TK-005 — … (Draft)`; cola de implementables sin TK-005. No implementa TK-005 hasta que su estado sea Ready.
  **Ejemplo 4 — Confirmación entre tareas**

- _Entrada:_ Hay tres TK Ready aprobadas.
- _Comportamiento:_ El agente implementa TK-001, actualiza `progress.md`, ejecuta lint, pregunta «¿Continúo con TK-002?». No avanza sin respuesta afirmativa.
